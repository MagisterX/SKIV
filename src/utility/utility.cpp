               ...
               UINT        flags)
{
  ...
  if ((flags & SEE_MASK_NOASYNC) == 0x0 &&
    ...
    flags   |= SEE_MASK_ASYNCOK; // Asynchronous - It is fine to defer loading the new process until later
...
SKIF_Util_GetControlledFolderAccess (void)
{
  ...
  if (ERROR_SUCCESS == RegOpenKeyExW (HKEY_LOCAL_MACHINE, LR"(SOFTWARE\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access\)", 0, KEY_READ | KEY_WOW64_64KEY, &hKey))
  {
    ...
    if (state)
    {
      ...
      {
        if (ERROR_SUCCESS == RegQueryValueEx (hKey, _path_cache.skiv_executable, NULL, NULL, NULL, NULL))
          state = 0;
...
SKIF_Util_RegisterApp (bool force)
{
  ...
  std::wstring wsExePath = std::wstring (_path_cache.skiv_executable);

  if (_registry.wsPathViewer      == _path_cache.skiv_install &&
      _registry.wsAppRegistration == wsExePath)
  {
    ret = 1;
  }

  else if (force || _registry.wsPathViewer.empty() || _registry.wsAppRegistration.empty())
  {
    ret = 1;
    ...
    {
      PLOG_ERROR << "Failed to register SKIV in Windows";
      ret = 0;
    }

    if (_registry.regKVPathViewer.putData (_path_cache.skiv_install))
      PLOG_INFO << "Updated central SKIV location: " << _path_cache.skiv_install;
    else
    {
      PLOG_ERROR << "Failed to update the central SKIV location!";
      ret = 0;
    }
  }
...
bool
SKIF_DirectoryWatch::isSignaled (void) const
{
  bool bRet = false;
...
LSTATUS
SKIF_RegistryWatch::registerNotify (void) const
{
